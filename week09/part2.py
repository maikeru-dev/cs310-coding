...
print(X_train.shape, X_test.shape, y_train.shape, y_test.shape)  # pyright: ignore
...
def modelling(X_train, y_train, X_test, y_test):
    ...
    tensorflow.config.threading.set_inter_op_parallelism_threads(16)
    # Create model
    model: Sequential = Sequential()  # pyright: ignore
    model.add(Dense(32, input_dim=n_features, activation="relu"))
    model.add(Dense(80, input_dim=n_features, activation="relu"))
    ...
    loss, acc = model.evaluate(X_test, y_test)  # pyright: ignore
...
def visualisation(X_test, y_test, model):
    # Evaluate
    error, mse = model.evaluate(X_test, y_test, verbose=0)
